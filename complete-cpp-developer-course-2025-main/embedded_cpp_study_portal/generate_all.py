...
def build_index():
    ...
    for p in SECTION_7_PROJECTS:
        ...
        all_projects.append({
            ...
            "rel_text": p['emb_badge'].replace("⚡ Embedded Relevance: ", ""),
            "rel_cls": p['emb_class'],
            "link": f"section_7/{p['id']}.html"
        })

    # Section 8 Projects
    for p in SECTION_8_PROJECTS:
        rel = "high" if "Critical" in p['emb_badge'] or "High" in p['emb_badge'] else ("med" if "Medium" in p['emb_badge'] else "core")
        all_projects.append({
            "sec": "8",
            "id": p['id'],
...
<head>
  ...
  <title>Modern C++ & Embedded Systems Deep-Dive Study Portal</title>
    ...
    <section class="hero">
      ...
      <div class="reference-banner">
        ...
        <div class="reference-links">
          ...
          <a href="https://github.com/mohamed-soubhi/The-Complete-Cpp-Developer-Course/blob/main/README.md" target="_blank" rel="noopener noreferrer" class="btn-ref">
...
def main():
    ...
    for idx, p in enumerate(SECTION_7_PROJECTS):
        ...
        next_p = SECTION_7_PROJECTS[idx + 1] if idx < len(SECTION_7_PROJECTS) - 1 else SECTION_7_PROJECTS[0]
        
        prev_link = f"{prev_p['id']}.html"
        next_link = f"{next_p['id']}.html"
        
        html_out = generate_page(p, prev_link, next_link, section_num=7)
        out_path = os.path.join(PORTAL_DIR, "section_7", f"{p['id']}.html")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(html_out)
        print(f"  ✓ [{idx+1}/9] section_7/{p['id']}.html")

    print("Generating Section 8 HTML pages...")
    for idx, p in enumerate(SECTION_8_PROJECTS):
        prev_p = SECTION_8_PROJECTS[idx - 1] if idx > 0 else SECTION_8_PROJECTS[-1]
        next_p = SECTION_8_PROJECTS[idx + 1] if idx < len(SECTION_8_PROJECTS) - 1 else SECTION_8_PROJECTS[0]
        
        prev_link = f"{prev_p['id']}.html"
        next_link = f"{next_p['id']}.html"
        
        html_out = generate_page(p, prev_link, next_link, section_num=8)
        out_path = os.path.join(PORTAL_DIR, "section_8", f"{p['id']}.html")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(html_out)
        print(f"  ✓ [{idx+1}/7] section_8/{p['id']}.html")

    print("Generating Section 11 HTML pages...")
    for idx, p in enumerate(SECTION_11_PROJECTS):
        prev_p = SECTION_11_PROJECTS[idx - 1] if idx > 0 else SECTION_11_PROJECTS[-1]
        next_p = SECTION_11_PROJECTS[idx + 1] if idx < len(SECTION_11_PROJECTS) - 1 else SECTION_11_PROJECTS[0]
        
        prev_link = f"{prev_p['id']}.html"
        next_link = f"{next_p['id']}.html"
        
        html_out = generate_page(p, prev_link, next_link, section_num=11)
        out_path = os.path.join(PORTAL_DIR, "section_11", f"{p['id']}.html")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(html_out)
        print(f"  ✓ [{idx+1}/19] section_11/{p['id']}.html")

    print("Generating Section 12 HTML pages...")
    for idx, p in enumerate(SECTION_12_PROJECTS):
        prev_p = SECTION_12_PROJECTS[idx - 1] if idx > 0 else SECTION_12_PROJECTS[-1]
        next_p = SECTION_12_PROJECTS[idx + 1] if idx < len(SECTION_12_PROJECTS) - 1 else SECTION_12_PROJECTS[0]
        
        prev_link = f"{prev_p['id']}.html"
        next_link = f"{next_p['id']}.html"
        
        html_out = generate_page(p, prev_link, next_link, section_num=12)
        out_path = os.path.join(PORTAL_DIR, "section_12", f"{p['id']}.html")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(html_out)
        print(f"  ✓ [{idx+1}/10] section_12/{p['id']}.html")

    print("Building Master Index Landing Page...")
    build_index()
    print("All portal pages generated successfully!")
